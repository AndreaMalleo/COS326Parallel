...
let filter (l:'a S.t ) (f: bool-> 'a -> 'a ): 'a S.t  =
   (*create bit vector for filter*)
    let bits = S.scan f true l in
    ...
    let coordsequence = S.tabulate (fun i ->
	  ...
	  Array.set (Array.get i bitsumarray) output) length in 
    coordsequence
...
let precompute (groups: group S.t) (us_area: area) (rows,cols) : int S.t S.t = 
 ...
 let popS  = S.map (fun g -> (g.pop,coord g)) groups  in
    ...
    let coordseq = filter sumcoords (fun bool i ->
          let (p,c,b) = i in
	  if (bool == b) then 1 else 0) in
